...
import { SocialLinkItem } from "@/lib/profile";
...
export const SocialLinks = memo(function SocialLinks({
  ...
  return (
    <nav
      className={`flex items-center justify-center flex-wrap gap-2.5 sm:gap-3 max-w-lg mx-auto ${className}`}
      aria-label="Social Profiles and Links"
    >
      {socials.map((item) => (
        <a
          key={item.name}
